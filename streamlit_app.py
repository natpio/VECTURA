...
try:
    ...
    st.error("🚨 KRYTYCZNY BŁĄD: BRAK BIBLIOTEK W SYSTEMIE")
    st.info("Sprawdź czy requirements.txt zawiera: st-gsheets-connection, streamlit, pandas, plotly, folium, streamlit-folium.")
    st.stop()

# --- 2. KONFIGURACJA WIZUALNA SQM MULTIMEDIA SOLUTIONS ---
st.set_page_config(
    page_title="SQM VECTURA | Logistics Tower", 
    layout="wide", 
    page_icon="🚛"
)
...
if not st.session_state["authenticated"]:
    ...
    st.markdown("System Logistyki Transportowej")
    pw = st.text_input("Podaj hasło systemowe:", type="password")
...
def load_data():
    try:
        # Odczyt danych z arkusza VECTURA
...
# DEFINICJA PEŁNEGO HARMONOGRAMU (10 ETAPÓW)
STAGES = [
...
# PRZETWARZANIE DAT I FINANSÓW (Zabezpieczenie przed NaT)
if not df.empty:
    for s in STAGES:
...
with tabs[0]:
    city_coords = {
        "Barcelona": [41.38, 2.17], "Berlin": [52.52, 13.40], "Poznań": [52.40, 16.92],
        "Dusseldorf": [51.22, 6.77], "Amsterdam": [52.36, 4.90], "Frankfurt": [50.11, 8.68],
        "Monachium": [48.13, 11.58], "Paryż": [48.85, 2.35], "Mediolan": [45.46, 9.19]
    }
    ...
    if not df.empty:
        for _, row in df.iterrows():
            ...
            folium.Marker(loc, popup=f"{row['Nazwa Targów']} | {row['Dane Auta']}", icon=folium.Icon(color=color, icon='truck', prefix='fa')).add_to(m)
        ...
        for idx, row in df.iterrows():
            st.markdown(f"""
                <div class="project-card">
                    ...
                    <p style="margin-top:12px; font-size:14px;">Kierowca: <b>{row.get('Kierowca','-')}</b> | Tel: <b>{row.get('Telefon','-')}</b></p>
                </div>
            """, unsafe_allow_html=True)
            
            # Wykres Gantta dla każdego projektu
            g_list = []
            ...
            if g_list:
                fig = px.timeline(pd.DataFrame(g_list), x_start="Start", x_end="Finish", y=[row['Nazwa Targów']]*len(g_list), color="Etap", template="plotly_white", color_discrete_map={s[0]: s[3] for s in STAGES})
...
# TAB 3: NOWE ZLECENIE (Wszystkie 19 kolumn)
with tabs[2]:
    with st.form("new_order_form"):
        ...
        st.subheader("Koszty i Personel")
        ...
        ki_n = c1.text_input("Kierowca")
        te_n = c2.text_input("Telefon")
        ...
        no_n = st.text_area("Notatki / Sloty")
        
        if st.form_submit_button("DODAJ DO BAZY"):
            if nt and da:
                new_data = pd.DataFrame([{
                    "Nazwa Targów": nt, "Logistyk": lg, "Dane Auta": da, "Kierowca": ki_n, "Telefon": te_n,
                    ...
                    "Odbiór Pełnych": hz5, "Trasa Powrót": hz5, "Rozładunek Powrotny": hz6, "Notatka": no_n
                }])
                updated = pd.concat([df.drop(columns=['Status Operacyjny','Suma Kosztów'], errors='ignore'), new_data], ignore_index=True)
                conn.update(worksheet="VECTURA", data=updated)
                st.success("Dodano zlecenie!"); time.sleep(1); st.rerun()

# TAB 4: PEŁNA EDYCJA (BEZ ŻADNYCH POMINIĘĆ)
with tabs[3]:
    if not df.empty:
        df['key'] = df['Nazwa Targów'] + " | " + df['Dane Auta']
        target = st.selectbox("Wybierz zlecenie do pełnej modyfikacji:", df['key'].unique())
        idx = df[df['key'] == target].index[0]
        r = df.loc[idx]
        
        with st.form("full_edit_form"):
            st.warning(f"Tryb edycji: {target}")
            e1, e2 = st.columns(2)
            e_nt = e1.text_input("Nazwa Targów", r['Nazwa Targów'])
            e_da = e2.text_input("Dane Auta", r['Dane Auta'])
            e_lg = e1.text_input("Logistyk", r['Logistyk'])
            e_ki = e2.text_input("Kierowca", r.get('Kierowca', ''))
            e_te = e1.text_input("Telefon", r.get('Telefon', ''))
            
            st.divider()
            st.subheader("Edycja Kosztów")
            f_e1, f_e2, f_e3 = st.columns(3)
            e_ex = f_e1.number_input("Eksport (PLN)", value=float(r['Koszt Eksport']))
            e_im = f_e2.number_input("Import (PLN)", value=float(r['Koszt Import']))
            e_ps = f_e3.number_input("Postoje (PLN)", value=float(r['Postoje i Parkingi']))
            
            st.divider()
            st.subheader("Edycja Harmonogramu")
            d_e1, d_e2, d_e3, d_e4 = st.columns(4)
            ed1 = d_e1.date_input("Załadunek SQM", get_safe_date(r['Data Załadunku']))
            ed2 = d_e2.date_input("Rozładunek Montaż", get_safe_date(r['Rozładunek Montaż']))
            ed3 = d_e3.date_input("Wjazd Empties", get_safe_date(r['Wjazd po Empties']))
            ed4 = d_e4.date_input("Rozładunek Powrotny", get_safe_date(r['Rozładunek Powrotny']))
            
            e_no = st.text_area("Notatka / Szczegóły Slotu", r['Notatka'])
            ...
            if st.form_submit_button("ZAPISZ WSZYSTKIE ZMIANY"):
                ...
                final_to_save = df.drop(columns=['Status Operacyjny','Suma Kosztów','key'], errors='ignore')
                conn.update(worksheet="VECTURA", data=final_to_save)
...
# TAB 5: BAZA DANYCH
with tabs[4]:
    st.subheader("Archiwum i Rejestr")
    st.dataframe(df.drop(columns=['key'], errors='ignore'), use_container_width=True)

# TAB 6: USUWANIE
with tabs[5]:
    if not df.empty:
        to_del = st.selectbox("Wybierz zlecenie do trwałego usunięcia:", df['key'].unique())
        if st.button("🚨 POTWIERDŹ USUNIĘCIE"):
            ...
            st.warning("Zlecenie usunięte."); time.sleep(1); st.rerun()
